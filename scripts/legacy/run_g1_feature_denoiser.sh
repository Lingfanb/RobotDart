#!/bin/bash
# Launch denoiser training on the 69-dim TextOp VAE.
# Stage 1 = pure denoise (no rollout), Stage 2/3 use single-step rollout
# (1 forward pass from pure noise) instead of full DDPM K-step loop.
# exp_name = g1_feature_mld → checkpoint at mld_denoiser/g1_feature_mld/
cd ~/Gitcode/DART
