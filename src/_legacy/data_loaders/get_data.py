...
from _legacy.data_loaders.tensors import collate as all_collate
from _legacy.data_loaders.tensors import t2m_collate
import torch
...
def get_dataset_class(name):
    if name == "amass":
        ...
        from _legacy.data_loaders.humanml.data.dataset import HumanML3D
        return HumanML3D
    elif name == "kit":
        from _legacy.data_loaders.humanml.data.dataset import KIT
        return KIT
    else:
...
def get_collate_fn(name, hml_mode='train'):
    if hml_mode == 'gt':
        from _legacy.data_loaders.humanml.data.dataset import collate_fn as t2m_eval_collate
...
def get_dataset_loader_mp(dataset_path, batch_size, split='train'):
    from _legacy.data_loaders.humanml.data.dataset import Text2MotionPrimitiveDataset
...
if __name__ == "__main__":
    ...
    from _legacy.data_loaders.humanml.data.dataset import PrimitiveSequenceDataset
