...
class TrainArgs:
    ...
    # Loss weights — 65-dim has explicit dof_velocity channel that must stay
    # physically consistent with dof_angle (decoupled in input → jerky output otherwise).
    ...
    weight_dof_smooth: float = 0.0     # 3rd-derivative (jerk) on DOF angles [11:40 in 69-dim, 7:36 in 65-dim]
                                        # Default 0 (off); set 0.5+ to penalize DOF jitter directly.
    weight_dof_vel_cons: float = 0.03  # NEW: enforce dof_velocity[t] ≈ diff(dof_angle[t]),
                                        # the 29-dim physical consistency between idx 7-35 and 36-64
...
class DenoiserMLPArgs:
    ...
    clip_dim: int = 512
    history_shape: tuple = (2, 65)
    noise_shape: tuple = (8, 65)
...
class G1FM65Args:
    ...
    """CLIP text encoder. ViT-B/32 = 512-dim (default), ViT-L/14 = 768-dim (friend's). Must match denoiser_args.model_args.clip_dim."""
    exp_name: str = "g1_fm_65_v1"
    seed: int = 0
    ...
    track: int = 1
    wandb_project_name: str = "g1_fm_65"
...
class G1FM65Trainer:
    ...
    ROOT_POSE_INDICES = ROOT_POSE_INDICES_65   # [0,1,2,3,4] = yaw_delta + transl_delta + root_height

    def __init__(self, args: G1FM65Args):
        self.args = args
        ...
        # Load 65-dim dataset (slices 69-dim pkl + decodes rp_trig→raw pitch/roll on load)
    ...
    def calc_loss(self, model_out, x0_gt, v_gt, x0_pred, history_motion=None):
        ...
        # DOF smoothness: 3rd-derivative (jerk) penalty on the 29 DOF angle channels.
        # This directly attacks "model amplifies底噪" jitter on joints. Apart from
        # root_smooth (which only covers root pose), this is the main lever to
        # smooth the actual joint motion that's visible in render.
        if x0_pred.shape[1] >= 4 and train_args.weight_dof_smooth > 0:
            pred_dof = x0_pred[..., DOF_ANGLE_SLICE_65]  # (B, T, 29) idx 7:36
            dof_jerk = (pred_dof[:, 3:] - 3 * pred_dof[:, 2:-1]
                        + 3 * pred_dof[:, 1:-2] - pred_dof[:, :-3])
            terms['dof_smooth'] = dof_jerk.pow(2).mean()
        else:
            terms['dof_smooth'] = torch.tensor(0.0, device=x0_pred.device)

        # DoF velocity-position consistency: dof_velocity[t] ≈ dof_angle[t+1] - dof_angle[t]
        # 65-dim feature has BOTH 29 angle channels (idx 7-35) and 29 velocity channels
        # (idx 36-64); without this loss the two channels can drift apart, causing jerky output.
        # Compare in NORMALIZED space directly (no need to denormalize for ratio consistency).
        if x0_pred.shape[1] >= 2 and train_args.weight_dof_vel_cons > 0:
            pred_dof_angle = x0_pred[..., DOF_ANGLE_SLICE_65]      # (B, T, 29) idx 7:36
            pred_dof_vel = x0_pred[..., DOF_VELOCITY_SLICE_65]      # (B, T, 29) idx 36:65
            calc_vel = pred_dof_angle[:, 1:, :] - pred_dof_angle[:, :-1, :]   # diff angles
            terms['dof_vel_cons'] = self.rec_criterion(calc_vel, pred_dof_vel[:, :-1, :])
        else:
            terms['dof_vel_cons'] = torch.tensor(0.0, device=x0_pred.device)
        ...
        total = (train_args.weight_x0_rec * primary_loss
                 ...
                 + train_args.weight_dof_smooth * terms['dof_smooth']
...
if __name__ == "__main__":
    args = tyro.cli(G1FM65Args)
    trainer = G1FM65Trainer(args)
    trainer.train()
    trainer.close()
