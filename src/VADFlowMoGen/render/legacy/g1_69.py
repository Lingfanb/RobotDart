"""Render text-conditioned G1 rollout from a 69-dim FM checkpoint.

69-dim features are frame-invariant: no recanonicalization needed between
...
Usage:
    MUJOCO_GL=egl python -m VADFlowMoGen.render.g1_35 \\
        --denoiser_checkpoint ./outputs/checkpoints/mld_denoiser/g1_fm_69_v1/checkpoint_280000.pt \\
...
from VADFlowMoGen.data.legacy.g1_69 import G1PrimitiveDataset69, FEATURE_DIM_69
from VADFlowMoGen.train.legacy.g1_69 import G1FM35Args, DenoiserMLPArgs
...
# ── 69-dim inverse: features → world motion ──────────────────────────────────

def inverse_features_69(features_np, init_yaw=0.0, init_xy=(0.0, 0.0)):
    """Convert (T, 69) TextOp features to world-space motion.
    Layout: rp_trig(0:4) + yaw_delta(4) + foot_contact(5:7) + transl_delta_local(7:10)
            + root_height(10) + dof_angle(11:40) + dof_velocity(40:69)
    foot_contact and dof_velocity are informational; not used for reconstruction.
    ...
    assert features_np.shape[1] == FEATURE_DIM_69

    # Decode rp_trig → roll, pitch
    sin_r = features_np[:, 0]
    cos_r = features_np[:, 1] + 1.0
    sin_p = features_np[:, 2]
    cos_p = features_np[:, 3] + 1.0
    roll = np.arctan2(sin_r, cos_r).astype(np.float32)
    pitch = np.arctan2(sin_p, cos_p).astype(np.float32)

    yaw_delta = features_np[:, 4]
    transl_delta = features_np[:, 7:10]   # dx, dy, dz in body-yaw frame
    root_height = features_np[:, 10]      # absolute z (preferred over integrating dz)
    dof = features_np[:, 11:40].astype(np.float32)
    # ignore foot_contact (5:7) and dof_velocity (40:69)

    # Accumulate yaw from yaw_delta
    yaw = np.zeros(T, dtype=np.float32)
    yaw[0] = init_yaw
    for t in range(1, T):
        yaw[t] = yaw[t - 1] + yaw_delta[t]

    # Accumulate xy from body-frame transl_delta (rotate by prev yaw back to world)
    ...
    # Use absolute root_height directly (more accurate than integrating dz)
    root_pos = np.stack([xy[:, 0], xy[:, 1], root_height.astype(np.float32)],
...
def plot_full_analysis(features_69, world_pos, yaw_all, dof_pos,
                       history_length, save_path, title):
    """Comprehensive analysis plot: root, euler, dof legs/arms, 69-dim features."""
    ...
    # Row 7: 69-dim raw features (root channels)
    ax = fig.add_subplot(gs[7, :])
    ch_names = ['yaw_vel', 'xy_vel_x', 'xy_vel_y', 'z', 'pitch', 'roll']
    for i in range(6):
        ax.plot(frames, features_69[:, i], label=ch_names[i], linewidth=1.2)
    ax.axvline(history_length - 0.5, color='red', linestyle='--', alpha=0.5)
    ax.set_title('69-dim features: root channels [0:6]')
...
def load_fm_35(checkpoint, device):
    """Load 69-dim FM checkpoint and return (denoiser_args, model, fm_sampler, full_args)."""
    ...
    print(f"Loaded 69-dim FM denoiser from {checkpoint} (step {ckpt.get('num_steps', '?')})")
...
@dataclass
class RenderArgs:
    denoiser_checkpoint: str = "./outputs/checkpoints/mld_denoiser/g1_fm_69_v1/checkpoint_280000.pt"
    prompts: tuple[str, ...] = (
...
def main():
    ...
    # Init from dataset (convert first sample to 69-dim for initial history)
    ...
    # Get initial 69-dim history (first H frames, already converted on device)
    ...
    init_features_69 = dataset.all_motion_tensor[args.init_idx]  # (T, D) NORMALIZED
    init_history_norm = init_features_69[:history_length, :].unsqueeze(0)
    # 69-dim dataset stores mean/std as (1,1,35); 2D input broadcasts to 3D.
    # Squeeze back to (H, 35) so it concats cleanly with future preds later.
    init_history_unnorm = dataset.denormalize(init_features_69[:history_length, :])
    ...
    for prompt in args.prompts:
        ...
        # Inverse: 69-dim -> world motion
        world_pos, root_quat_wxyz, dof_pos = inverse_features_69(
