...
Usage in trainer:
    from VADFlowMoGen.data.legacy.g1_37_va import G1PrimitiveDataset37VA

    train = G1PrimitiveDataset37VA(
...
# ── 37-dim feature layout (35-dim base + 2 foot_contact at end) ──────────
# idx 0:    yaw_vel
...
# idx 5:    roll
# idx 6-34: dof_pos (29)
# idx 35:36: foot_contact (left, right) ← computed FK-style from link_pos_local
MOTION_DIM_V2 = 37
DOF = 29

# G1 ankle indices in 29-link layout (matches utils/g1_utils.py)
G1_LEFT_ANKLE_IDX = 5     # left_ankle_roll_link
G1_RIGHT_ANKLE_IDX = 11   # right_ankle_roll_link
FOOT_CONTACT_Z_THRESH = 0.05   # binary contact: world ankle z < 5cm = on ground


def compute_foot_contact_np(root_pos: np.ndarray,
                             root_quat_wxyz: np.ndarray,
                             link_pos_local: np.ndarray,
                             threshold: float = FOOT_CONTACT_Z_THRESH) -> np.ndarray:
    """Binary (left, right) foot_contact from world-frame ankle z. Same logic as
    G1PrimitiveUtility69.compute_foot_contact_world but in numpy.

    Args:
        root_pos:        (T, 3)   world position
        root_quat_wxyz:  (T, 4)   world rotation, wxyz
        link_pos_local:  (T, 29, 3)   ankle positions in pelvis-local frame
    Returns:
        (T, 2) float32, {0.0, 1.0}, order [left, right]
    """
    q_xyzw = root_quat_wxyz[:, [1, 2, 3, 0]].astype(np.float32)
    R_mat = R.from_quat(q_xyzw).as_matrix().astype(np.float32)   # (T, 3, 3)
    ankle_local = link_pos_local[:, [G1_LEFT_ANKLE_IDX, G1_RIGHT_ANKLE_IDX], :].astype(np.float32)
    world_ankle = np.einsum('tij,tnj->tni', R_mat, ankle_local) + root_pos[:, None, :].astype(np.float32)
    z = world_ankle[..., 2]   # (T, 2)
    return (z < threshold).astype(np.float32)
...
def extract_features_37(dof_pos: np.ndarray,
                         root_pos: np.ndarray,
                         root_quat: np.ndarray,
                         link_pos_local: np.ndarray) -> np.ndarray:
    """Convert raw motion to 37-dim feature: 35-dim base + 2 foot_contact at end."""
    feat35 = _extract_features_35_core(dof_pos, root_pos, root_quat)
    contact = compute_foot_contact_np(root_pos, root_quat, link_pos_local)
    feat37 = np.concatenate([feat35, contact], axis=-1).astype(np.float32)
    assert feat37.shape == (len(dof_pos), MOTION_DIM_V2)
    return feat37


def _extract_features_35_core(dof_pos: np.ndarray,
                               root_pos: np.ndarray,
                               root_quat: np.ndarray) -> np.ndarray:
    """35-dim core (same formula as VA's extract_features_v2)."""
    T = len(dof_pos)
    ...
    assert feats.shape == (T, 35)   # core is 35-dim, foot_contact appended later
    return feats
    ...
    def __init__(self,
        ...
        for fname in tqdm(npz_files, desc=f"Load VA NPZ [{split}]"):
            try:
                ...
                if 'link_pos_local' not in d.files:
                    skipped += 1; continue   # 37-dim needs link_pos_local for FK contact
                lpl_src = d['link_pos_local'].astype(np.float32)   # (T, 29, 3)
                T_src = len(dof_src)
                if T_src < 4:
                    skipped += 1; continue
                ...
                # link_pos_local is (T,29,3); flatten 87→resample→reshape (no-op when src==tgt fps)
                if abs(source_fps - target_fps) < 1e-6:
                    lpl_t = lpl_src
                else:
                    flat = lpl_src.reshape(T_src, -1)   # (T, 87)
                    flat_r = _resample_linear(flat, source_fps, target_fps)
                    lpl_t = flat_r.reshape(-1, 29, 3).astype(np.float32)
                T_tgt = min(len(dof_t), len(rp_t), len(rq_t), len(lpl_t))
                if T_tgt < self.primitive_length:
                    skipped += 1; continue

                feats = extract_features_37(dof_t[:T_tgt], rp_t[:T_tgt],
                                              rq_t[:T_tgt], lpl_t[:T_tgt])
        ...
        print(f"[VA-NPZ {split}] loaded {len(sequences)} clips, skipped {skipped}, "
              f"resampled {source_fps}→{target_fps} fps, feature_dim=37")
        ...
        N = len(sequences)
        max_T = max(len(s['features_37']) for s in sequences)
        self.all_motion_tensor = torch.zeros(N, max_T, 37, dtype=torch.float32)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        npz_dir = sys.argv[1]
        ds = G1PrimitiveDataset37VA(npz_dir=npz_dir, split='train',
        ...
        print("Usage: python dataset_g1_37_va.py <npz_dir>")
