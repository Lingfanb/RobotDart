"""G1 69-dim Dataset — full TextOp / DART representation, identity load.

Layout (69-dim, matches TextOp):
  [0:4]    root_rp_trig (sin_roll, cos_roll-1, sin_pitch, cos_pitch-1)
  [4]      yaw_delta
  [5:7]    foot_contact (left, right)
  [7:10]   transl_delta_local (dx, dy, dz)
  [10]     root_height
  [11:40]  dof_angle (29)
  [40:69]  dof_velocity (29)

Reads pkl from mp_data_g1_69/ — passes features_69 through unchanged. No conversion.

Note: dof_velocity (29 ch) was shown to hurt FM in Exp 21 (65→35 -30% sf). This
69-dim variant is for completeness (TextOp original recipe verification, paper §4).
"""
import os
import json
import pickle
...
def convert_69_to_69(feat69: np.ndarray) -> np.ndarray:
    """Identity passthrough — features_69 from pkl is already TextOp 69-dim."""
    assert feat69.shape[-1] == FEATURE_DIM_69, f"got {feat69.shape[-1]}, expected 69"
    return feat69.astype(np.float32)


class G1PrimitiveDataset69:
    """69-dim frame-invariant G1 motion primitive dataset.

    Reads 69-dim pkl data and converts to 69-dim on load.  Provides the same
    interface as G1PrimitiveSequenceDataset (get_batch, normalize, denormalize,
    weighted sampling, CLIP text embeddings).
    """

    def __init__(self, dataset_name='g1_mp_69',
        ...
        self.feature_dim = FEATURE_DIM_69
        ...
        assert '69dim' in self.feature_version, \
            f"69-dim dataset requires 69-dim source data, got {self.feature_version}"
        ...
        print(f'G1 69-dim dataset: H={self.history_length}, F={self.future_length}, '
        ...
        print(f'G1 69-dim Dataset [{split}]: {len(self.dataset)} primitives (from 69-dim pkl)')
        ...
        # ── Stack features_69 directly (identity passthrough, no conversion) ──
        print(f'  [{split}] Loading {len(self.dataset)} primitives as 69-dim ...')
        all_69 = np.stack([d['features_69'] for d in self.dataset], axis=0).astype(np.float32)  # (N, T, 69)
        assert all_69.shape[-1] == FEATURE_DIM_69
        self.all_motion_tensor = torch.from_numpy(all_69).float().to(self.device)
        ...
        # ── Compute or load 69-dim mean/std ──
        mean_std_path = pjoin(dataset_path, 'mean_std_69.pkl')
        if os.path.exists(mean_std_path) and split == 'train':
            ...
            print(f'  Loaded 69-dim mean/std from {mean_std_path}')
        elif split == 'train':
            print(f'  Computing 69-dim mean/std (first run)...')
            flat = self.all_motion_tensor.reshape(-1, FEATURE_DIM_69)  # (N*T, 35)
            ...
            print(f'  Saved 69-dim mean/std to {mean_std_path}')
        else:
            ...
            if os.path.exists(mean_std_path):
                ...
                raise FileNotFoundError(
                    f"mean_std_69.pkl not found. Run with split='train' first to compute it.")
