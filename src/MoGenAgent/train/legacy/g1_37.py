...
from MoGenAgent.data.legacy.g1_37 import G1PrimitiveDataset37, FEATURE_DIM_37
...
class DenoiserMLPArgs:
    ...
    clip_dim: int = 512
    history_shape: tuple = (2, 35)
    noise_shape: tuple = (8, 35)
...
class G1FM35Args:
    ...
    data_source: str = "dart_pkl"
    """Data source: 'dart_pkl' (our mp_data_g1_69 pkl) or 'va_npz' (VA's NPZ-per-clip directory).
    For va_npz, set data_dir to the NPZ root (e.g. balanced_reassembled_sim/successful)."""
    source_fps: float = 20.0
    """For va_npz only: source data fps (VA is 20fps). Resampled to 30fps on load."""
    num_primitive: int = 4
    """consecutive primitives per training step"""
    exp_name: str = "g1_fm_37_v1"
    seed: int = 0
    ...
    track: int = 1
    wandb_project_name: str = "g1_fm_37"
...
class G1FM35Trainer:
    ...
    ROOT_POSE_INDICES = [0, 1, 2, 3, 4, 5]

    def __init__(self, args: G1FM35Args):
        self.args = args
        ...
        if args.data_source == 'va_npz':
            ...
            train_dataset = G1PrimitiveDataset37VA(
                ...
                num_primitive=args.num_primitive,
                weight_scheme='text',
                mean_std_path=str(args.save_dir / 'mean_std_va_train.pkl'))
            val_dataset = G1PrimitiveDataset37VA(
                npz_dir=args.data_dir, split='val', device=device,
                source_fps=args.source_fps, target_fps=30.0,
                history_length=2, future_length=8,
                num_primitive=1,
                weight_scheme='uniform',
                mean_std_path=str(args.save_dir / 'mean_std_va_train.pkl'))
        elif args.data_source == 'dart_pkl':
            train_dataset = G1PrimitiveDataset37(
                dataset_path=args.data_dir, split='train', device=device,
                weight_scheme='text', num_primitive=args.num_primitive)
            val_dataset = G1PrimitiveDataset37(
                dataset_path=args.data_dir, split='val', device=device,
                weight_scheme='uniform', num_primitive=1)
        else:
            raise ValueError(f"Unknown data_source: {args.data_source} (use 'dart_pkl' or 'va_npz')")
...
if __name__ == "__main__":
    args = tyro.cli(G1FM35Args)
    trainer = G1FM35Trainer(args)
    trainer.train()
    trainer.close()
